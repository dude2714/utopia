...
def addon_fanart(fallback='Utopia_fanart.png'):
	fanart = get_setting('fanart_image', fallback)
	if fanart == 'Utopia_fanart.png': return kodi_utils.get_addoninfo('fanart')
...
def auto_start_Utopia():
	return get_setting('auto_start_Utopia') == 'true'
...
def nextep_content_settings():
	...
	sort_key = 'Utopia_last_played' if sort_type == 0 else 'Utopia_first_aired' if sort_type == 1 else 'Utopia_name'
...
def watched_title(watched_indicators):
	return ('Utopia', 'Trakt', 'MDBList')[watched_indicators]
