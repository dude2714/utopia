...
UtopiaPlayer, progressDialogBG, notification = player.UtopiaPlayer, kodi_utils.progressDialogBG, kodi_utils.notification
...
class Sources:
	def __init__(self):
		...
		self.progress_dialog, self.Utopia_background_url = None, None
...
#			if not self.debrid_torrent_enabled: self.exclude_list.extend(scraper_names('torrents'))
	...
	def play_source(self, results):
		if self.background: self.Utopia_background_url = self.play_file(results, autoplay=True, background=True)
...
#		return cached + uncached
	...
	def background_prep(cls, params):
		...
		return self.Utopia_background_url
