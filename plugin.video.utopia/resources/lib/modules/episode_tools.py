...
def get_random_episode(tmdb_id, continual=False):
	...
	try:
		...
		episodes_data = [i for i in episodes_data if i['season'] != 0 and adjust_premiered_date(i['premiered'], adjust_hours)[0] <= current_date]
	...
	if continual:
		...
		episode_list = []
		try:
			episode_history = json.loads(kodi_utils.get_property('Utopia_random_episode_history'))
	...
	if continual:
		...
		kodi_utils.set_property('Utopia_random_episode_history', json.dumps(episode_history))
...
def execute_nextep(meta, nextep_settings):
	...
	def _confirm_threshold():
		...
		try: current_number = int(kodi_utils.get_property('Utopia_total_autoplays'))
		except: current_number = 1
		if current_number < nextep_threshold:
			current_number += 1
			kodi_utils.set_property('Utopia_total_autoplays', str(current_number))
			return True
	...
	if action == 'cancel':
		Sources.nextep_params.clear()
		kodi_utils.clear_property('Utopia_total_autoplays')
		return kodi_utils.notification(32736)
	if action == 'close':
		if run_popup: return
