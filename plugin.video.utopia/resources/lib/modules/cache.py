...
def check_databases():
	...
	dbcon.execute("""CREATE INDEX IF NOT EXISTS Utopia_select_id_media ON metadata (tmdb_id, db_type)""")
	dbcon.close()
	...
	dbcon.execute("""CREATE INDEX IF NOT EXISTS Utopia_ws_in_progress_episodes ON watched_status (db_type, media_id, season DESC, episode DESC)""")
	dbcon.close()
...
def remove_old_packages():
	...
	for item in files:
		if '.Utopia' in item and item.endswith('zip'):
...
def clear_all_cache():
	...
	kodi_utils.progressDialog.create('Utopia', '')
