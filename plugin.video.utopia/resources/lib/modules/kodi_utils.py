...
navigator_db   = 'special://profile/addon_data/plugin.video.utopia/navigator.db'
watched_db     = 'special://profile/addon_data/plugin.video.utopia/watched.db'
favorites_db   = 'special://profile/addon_data/plugin.video.utopia/watched.db'
views_db       = 'special://profile/addon_data/plugin.video.utopia/views.db'
trakt_db       = 'special://profile/addon_data/plugin.video.utopia/traktcache.db'
mdbl_db        = 'special://profile/addon_data/plugin.video.utopia/mdblcache.db'
maincache_db   = 'special://profile/addon_data/plugin.video.utopia/maincache.db'
metacache_db   = 'special://profile/addon_data/plugin.video.utopia/metacache.db'
debridcache_db = 'special://profile/addon_data/plugin.video.utopia/debridcache.db'
external_db    = 'special://profile/addon_data/plugin.video.utopia/providerscache.db'
databases_path = 'special://profile/addon_data/plugin.video.utopia/'
...
def media_path(*args):
	path = 'special://home/addons/plugin.video.utopia/resources/skins/Default/media/'
...
def addon(addon_id='plugin.video.utopia'):
...
def end_directory(handle, cacheToDisc=None):
	if cacheToDisc is None: cacheToDisc = get_property('Utopia_kodi_menu_cache') == 'true'
...
def external_browse():
	return 'Utopia' not in get_infolabel('Container.PluginName')
...
def ok_dialog(heading='Utopia', text='', highlight='dodgerblue', ok_label=local_string(32839), top_space=False):
...
def confirm_dialog(heading='Utopia', text='', highlight='dodgerblue', ok_label=local_string(32839), cancel_label=local_string(32840), top_space=False, default_control=11):
#	from windows import open_window
	if isinstance(heading, int): heading = local_string(heading)
	if isinstance(text, int): text = local_string(text)
...
def notification(line1, time=3000, icon=None, sound=False):
	...
	dialog.notification('Utopia', line1, icon, time, sound)
...
def set_view_property(view_type, view_id):
	set_property('Utopia_%s' % view_type, view_id)
...
def set_view_properties():
	...
	for item in view_ids: set_property('Utopia_%s' % item[0], item[1])

def set_view_mode(view_type, content='files'):
	if external_browse(): return
	view_id = get_property('Utopia_%s' % view_type)
	hold = 0
	if not view_id:
		try:
			dbcon = database_connect(views_db, isolation_level=None)
...
def clear_view(view_type):
	...
	try:
		...
		for item in dbcur.fetchall():
			...
			clear_property('Utopia_%s' % item[0])
		dbcon = database_connect('special://profile/Database/ViewModes6.db')
		dbcur = dbcon.cursor()
		dbcur.execute("""DELETE FROM view WHERE path LIKE 'plugin://plugin.video.utopia/%'""")
		dbcon.commit()
		dbcon.close()
...
def get_setting(setting_id, fallback=None):
	try: settings_dict = json.loads(get_property('Utopia_settings'))
...
def make_settings_dict():
	...
	settings_dict = None
	try:
		profile_dir = 'special://profile/addon_data/plugin.video.utopia/'
		...
		set_property('Utopia_settings', json.dumps(settings_dict))
	except Exception as e: logger('make_settings_dict error', str(e))
	return settings_dict

def clean_settings():
	import xml.etree.ElementTree as ET
	addon_ids = 'plugin.video.utopia'
...
def open_settings(query, addon='plugin.video.utopia'):
	hide_busy_dialog()
	if query:
		try:
