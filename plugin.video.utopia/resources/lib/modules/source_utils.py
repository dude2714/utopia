...
ADS = ('1xbet', 'betwin')
try: UNWANTED_TAGS = json.loads(kodi_utils.get_property('Utopia_unwanted'))['unwanted']
except: UNWANTED_TAGS = []

def internal_sources(active_sources, mediatype, prescrape=False):
	source_list = []
	append = source_list.append
	files = kodi_utils.list_dirs('special://home/addons/plugin.video.utopia/resources/lib/scrapers')[1]
	for item in files:
		try:
...
def enable_disable(folder):
	try:
		icon = 'special://home/addons/plugin.video.utopia/resources/lib/fenom/fenom_icon.png'
...
def scraper_names(folder):
	...
	source_folder_location = 'special://home/addons/plugin.video.utopia/resources/lib/magneto/%s'
