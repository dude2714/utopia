...
def set_quality_choice(quality_setting):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Utopia', 'multi_choice': 'true', 'preselect': preselect}
...
def results_sorting_choice():
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Utopia'}
...
def color_choice(msg_dialog='Utopia', no_color=False):
...
def favorites_choice(params):
	...
	if params.get('cache'):
		...
		if tmdb_id in {i['tmdb_id'] for i in current_favorites}:
			...
			text = '%s Utopia %s?' % (ls(32603), ls(32453))
		else: action, text = favorites.add, '%s Utopia %s?' % (ls(32602), ls(32453))
...
def dropped_choice(params):
	...
	if tmdb_id in {int(i['tmdb_id']) for i in current_favorites}:
		action, text = dropped.remove, '%s Utopia %s?' % (ls(32603), 'Dropped')
	else: action, text = dropped.add, '%s Utopia %s?' % (ls(32602), 'Dropped')
	if not confirm_dialog(text='%s[CR][CR]%s' % (title, text)): return
	notification(32576) if action(mediatype, tmdb_id, title) else notification(32574)
	container_refresh()
...
def options_menu(params, meta=None):
	...
	listing = (
		...
		('open_Utopia_settings', '%s %s %s' % (open_str, ls(32036), settings_str), ''),
		('reload_widgets', 'Utopia: Refresh Widgets', '') if is_widget else None
	)
	...
	elif choice == 'open_Utopia_settings': return kodi_utils.open_settings('0.0')
