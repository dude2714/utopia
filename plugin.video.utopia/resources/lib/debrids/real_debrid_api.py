...
class RealDebridAPI:
	...
	def clear_cache(*args):
		...
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_rd_user_cloud',))
				clear_property('Utopia_rd_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_rd_downloads',))
				clear_property('Utopia_rd_downloads')
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_rd_valid_hosts',))
				clear_property('Utopia_rd_valid_hosts')
				dbcon.commit()
