...
class AllDebridAPI:
	...
	def clear_cache(self):
		...
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_ad_user_cloud',))
				clear_property('Utopia_ad_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_ad_downloads',))
				clear_property('Utopia_ad_downloads')
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_ad_valid_hosts',))
				clear_property('Utopia_ad_valid_hosts')
				dbcon.commit()
