...
user_agent = 'Utopia/%s' % kodi_utils.get_addoninfo('version')
client_id = '384733001'
...
#			transfer_id = result['id']
	...
	def get_item_details(self, item_id):
		string = 'Utopia_pm_item_details_%s' % item_id
		url = 'item/details'
		data = {'id': item_id}
		args = [url, data]
	...
	def clear_cache(*args):
		...
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('Utopia_pm_user_cloud%',))
				user_cloud_cache = [str(i[0]) for i in dbcur.fetchall()]
				if user_cloud_cache:
					dbcur.execute("""DELETE FROM maincache WHERE id LIKE ?""", ('Utopia_pm_user_cloud%',))
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_pm_downloads',))
				clear_property('Utopia_pm_downloads')
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id = ?""", ('Utopia_pm_valid_hosts',))
				clear_property('Utopia_pm_valid_hosts')
				dbcon.commit()
