...
try: UNDESIRABLES = jsloads(homeWindow.getProperty('Utopia_unwanted'))['undesirables']
except: UNDESIRABLES = []
...
try: unwanted_tags = jsloads(homeWindow.getProperty('Utopia_unwanted'))['unwanted']
...
def clean_name(release_title):
	try:
		release_title = re.sub(r'ã€.*?ã€‘', '', release_title)
