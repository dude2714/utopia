...
class Navigator:
	...
	def settings(self):
		Utopia_str, manager_str, changelog_str, short_str = ls(32036), ls(32513), ls(32508), ls(32514)
		...
		self._add_item({'mode': 'open_settings', 'query': '4.0', 'name': Utopia_str                 }, 'Utopia.png', n_ins, False)
	...
	def log_utils(self):
		Utopia_vstr, log_path = ku.get_addoninfo('version'), 'special://home/addons/%s/changelog.txt'
		kl_loc, mt_str = 'special://logpath/kodi.log', log_path % 'plugin.video.utopia'
		Utopia_str, cl_str, lut_str, k_str, lv_str = ls(32036), ls(32508), ls(32777), ls(32538), ls(32509)
		mh_str, klv_h, klu_h = '%s  [I](v.%s)[/I]' % (Utopia_str, Utopia_vstr), '%s %s' % (k_str, lv_str), ls(32853)
	...
	def because_you_watched(self):
		...
		def _convert_Utopia_watched_episodes_info(watched_indicators):
...
#			_watched.sort(key=lambda x: (x[0], x[1], x[2]), reverse=True)
		...
		function = get_watched_info_movie if mediatype == 'movie' else _convert_Utopia_watched_episodes_info
		...
		def build_main_lists():
			for item_position, item in enumerate(contents):
				try:
					...
					elif item_get('iconImage') == 'Utopia.png': icon = ku.get_addoninfo('icon')
	...
	def _add_item(self, url_params, iconImage='', prefix='', isFolder=True, list_name=''):
		...
		elif iconImage == 'Utopia.png': icon = ku.get_addoninfo('icon')
