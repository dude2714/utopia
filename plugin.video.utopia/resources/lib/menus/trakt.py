...
def search_trakt_lists(params):
	...
	search_title = params.get('search_title') or kodi_utils.dialog.input('Utopia')
...
def build_trakt_list(params):
	...
	max_threads = int(kodi_utils.get_setting('Utopia.max_threads', '100'))
	...
	items.sort(key=lambda k: int(k[1].getProperty('Utopia_sort_order')))
	content, total = max(
