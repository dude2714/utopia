...
class Router:
	...
	def __exit__(self, exc_type, exc_value, traceback):
		if get_property('Utopia_rli_fix') == 'true' and external_browse():
			message = f"Utopia not in '{get_infolabel('Container.PluginName')}'"
...
class UtopiaMonitor(kodi_utils.xbmc_monitor):
	def __enter__(self):
	...
	def onScreensaverActivated(self):
		set_property('Utopia_pause_services', 'true')

	def onScreensaverDeactivated(self):
		clear_property('Utopia_pause_services')

	def onSettingsChanged(self):
		clear_property('Utopia_settings')
		kodi_utils.sleep(50)
		make_settings_dict()
		set_property('Utopia_kodi_menu_cache', get_setting('kodi_menu_cache'))
		set_property('Utopia_rli_fix', get_setting('rli_fix'))

	def onNotification(self, sender, method, data):
		if method == 'System.OnSleep': set_property('Utopia_pause_services', 'true')
		elif method == 'System.OnWake': clear_property('Utopia_pause_services')

def initializeDatabases():
	from modules.cache import check_databases
	logger('Utopia', 'InitializeDatabases Service Starting')
	check_databases()
	return logger('Utopia', 'InitializeDatabases Service Finished')

def checkSettingsFile():
	logger('Utopia', 'CheckSettingsFile Service Starting')
	clear_property('Utopia_settings')
	...
	set_property('Utopia_kodi_menu_cache', get_setting('kodi_menu_cache'))
	set_property('Utopia_rli_fix', get_setting('rli_fix'))
	return logger('Utopia', 'CheckSettingsFile Service Finished')
...
def databaseMaintenance():
	...
	logger('Utopia', 'Database Maintenance Service Starting')
	clean_databases(current_time, database_check=False, silent=True)
	set_setting('database.maintenance.due', str(next_clean))
	return logger('Utopia', 'Database Maintenance Service Finished')

def viewsSetWindowProperties():
	logger('Utopia', 'ViewsSetWindowProperties Service Starting')
	kodi_utils.set_view_properties()
	return logger('Utopia', 'ViewsSetWindowProperties Service Finished')

def reuseLanguageInvokerCheck():
	import xml.etree.ElementTree as ET
	logger('Utopia', 'ReuseLanguageInvokerCheck Service Starting')
	addon_xml = translate_path('special://home/addons/plugin.video.utopia/addon.xml')
	tree = ET.parse(addon_xml)
	root = tree.getroot()
	current_addon_setting = get_setting('reuse_language_invoker', 'true')
	text = '[B]Reuse Language Invoker[/B] SETTING/XML mismatch[CR]Utopia will reload your profile to refresh the addon.xml'
	...
	return logger('Utopia', 'ReuseLanguageInvokerCheck Service Finished')

def autoRun():
	logger('Utopia', 'AutoRun Service Starting')
	if settings.auto_start_Utopia(): kodi_utils.execute_builtin('RunAddon(plugin.video.utopia)')
	return logger('Utopia', 'AutoRun Service Finished')

def clearSubs():
	logger('Utopia', 'Clear Subtitles Service Starting')
	sub_formats = ('.srt', '.ssa', '.smi', '.sub', '.idx')
	subtitle_path = 'special://temp/'
	for i in kodi_utils.list_dirs(subtitle_path)[1]:
		if i.startswith('UtopiaSubs_') or i.endswith(sub_formats):
			kodi_utils.delete_file(subtitle_path + i)
	return logger('Utopia', 'Clear Subtitles Service Finished')
...
def traktMonitor():
	...
	logger('Utopia', 'TraktMonitor Service Starting')
	trakt_service_string = 'TraktMonitor Service Update %s - %s'
	update_string = 'Next Update in %s minutes...'
	if get_property('Utopia_traktmonitor_first_run') != 'true':
		for i in ('user_lists', 'liked_lists', 'my_lists'): clear_trakt_list_contents_data(i)
		clear_tmdbl_cache()
		set_property('Utopia_traktmonitor_first_run', 'true')
	while not monitor.abortRequested():
		while is_playing() or get_visibility('Container().isUpdating') or get_property('Utopia_pause_services') == 'true':
		...
		if status == 'success':
			logger('Utopia', trakt_service_string % ('Utopia TraktMonitor - Success', 'Trakt Update Performed'))
			if settings.trakt_sync_refresh_widgets():
				kodi_utils.widget_refresh()
				logger('Utopia', trakt_service_string % ('Utopia TraktMonitor - Widgets Refresh', 'Setting Activated. Widget Refresh Performed'))
			else: logger('Utopia', trakt_service_string % ('Utopia TraktMonitor - Widgets Refresh', 'Setting Disabled. Skipping Widget Refresh'))
		elif status == 'no account':
			logger('Utopia', trakt_service_string % ('Utopia TraktMonitor - Aborted. No Trakt Account Active', next_update_string))
		elif status == 'failed':
			logger('Utopia', trakt_service_string % ('Utopia TraktMonitor - Failed. Error from Trakt', next_update_string))
		else:# 'not needed'
			logger('Utopia', trakt_service_string % ('Utopia TraktMonitor - Success. No Changes Needed', next_update_string))
		try: status = mdbl_sync_activities()
		except: status = 'failed'
		if status == 'success':
			logger('Utopia', trakt_service_string % ('Utopia MDBListMonitor - Success', 'MDBList Update Performed'))
			if settings.trakt_sync_refresh_widgets():
				kodi_utils.widget_refresh()
				logger('Utopia', trakt_service_string % ('Utopia MDBListMonitor - Widgets Refresh', 'Setting Activated. Widget Refresh Performed'))
			else: logger('Utopia', trakt_service_string % ('Utopia MDBListMonitor - Widgets Refresh', 'Setting Disabled. Skipping Widget Refresh'))
		elif status == 'no account':
			logger('Utopia', trakt_service_string % ('Utopia MDBListMonitor - Aborted. No MDBList Account Active', next_update_string))
		elif status == 'failed':
			logger('Utopia', trakt_service_string % ('Utopia MDBListMonitor - Failed. Error from MDBList', next_update_string))
		else:# 'not needed'
			logger('Utopia', trakt_service_string % ('Utopia MDBListMonitor - Success. No Changes Needed', next_update_string))
		try:
			if get_setting('tmdb.token') and get_setting('tmdblist.watchlist_sync') == 'true':
				status = tmdb_clean_watchlist(silent=True)
				if status: logger('Utopia', 'TMDB Lists Service Update - Success. %s' % status)
		except: pass
		monitor.waitForAbort(interval)
	return logger('Utopia', 'TraktMonitor Service Finished')

def premAccntNotification():
	logger('Utopia', 'Debrid Account Expiry Notification Service Starting')
	...
	return logger('Utopia', 'Debrid Account Expiry Notification Service Finished')

def checkUndesirablesDatabase():
	logger('Utopia', 'CheckUndesirablesDatabase Service Starting')
	path = 'special://home/addons/%s/resources/unwanted.json' % kodi_utils.get_addoninfo('id')
	with kodi_utils.open_file(path) as file: set_property('Utopia_unwanted', str(file.read()))
	...
	return logger('Utopia', 'CheckUndesirablesDatabase Service Finished')
